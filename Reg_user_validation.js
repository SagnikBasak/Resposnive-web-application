...
//Confirm password & password field
  ...
  if(pass.length<5){
    document.getElementById('pass_valid').innerHTML ="less thn 5";
    return false;
  }
  if(pass.length>20){
    document.getElementById('pass_valid').innerHTML ="greater thn 20";
    return false;
  }
  if(pass.match(/[0-9]/) < 1){
    document.getElementById('pass_valid').innerHTML ="Contains numerical character";
    return false;
  }
  if(pass.match(/[^A-Za-z0-9-' ']/) < 1){
    document.getElementById('pass_valid').innerHTML ="Contains special character";
    return false;
  }
  if (pass.match(/^(?=.*\s)/)){
    document.getElementById('pass_valid').innerHTML ="Shouldn't contain spaces";
    return false;
  }
  if(pass == ""){
    document.getElementById('pass_valid').innerHTML ="Password Shouldn't be empty";
    return false;
  }
}
